...
/**
 * Put `collections` on the fixture's first network. Spreading `personhood` and adding a
 * `collections` key lands one level too high, which `.strict()` reports as an unrecognised key --
 * passing a `toThrow` for the wrong reason.
 */
...
describe('parseConfig', () => {
  ...
  /**
   * `trusted_equally` records a fact the service cannot check, so these tests are about it being
   * unavoidable rather than correct.
   */
  describe('auth.personhood.networks', () => {
    ...
    it('refuses several networks unless the operator asserts they are equally hard to register on', () => {
      // The gate is only as strong as the weakest network listed, and that cannot be measured
      // here. The assertion must not be reachable by leaving a default alone.
    ...
    it('accepts the same collection identifier on two networks, which is the live shape', () => {
      // All three environments publish `people-lite` under a byte-identical id (verified live).
      expect(() =>
    ...
    it('names the offending network by index when one endpoint is plaintext', () => {
      // One plaintext RPC among several is the whole hole, the caller choosing which answers.
      expect(() =>
        parseConfig(
          rawConfig({
            environment: 'production',
