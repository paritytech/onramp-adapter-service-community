 ...
 * The `net` claim.
 *
 * It is the only durable record of which chain admitted a person -- the alias deliberately cannot
 * say, being chain-independent -- so it is verified as strictly as `sub` and `aud`.
 */
describe('the network claim', () => {
  ...
  it('refuses a token carrying no network, rather than defaulting one', async () => {
    // A token minted before this claim existed verifies cleanly: same key, same algorithm. So a
    // default would file those rows in the audit trail under a chain nobody observed, during
    // exactly the rollout window where the record matters. One TTL of 401s is the honest cost.
